...
int cmd_util(int argc, char **argv)
{
    char buf[512];
    vTaskGetRunTimeStats(&buf[0]);
    printf("Task            Cycles          Utilization\r\n");
    printf("===========================================\r\n");
    printf("%s\r\n", buf);

    return 0;
}
SHELL_CMD_EXPORT_ALIAS(cmd_util, util, show utilization usage);

int cmd_net(int argc, char argv) {

    return 0;
}
SHELL_CMD_EXPORT_ALIAS(cmd_net, net, network commands);


int cmd_ifconfig(int argc, char argv) {
    struct netif *port;
    for (uint8_t i = 0; i < net_get_port_count(); i++) {
        port = net_get_port(i);
        if (port == NULL) {
            LOG_E("Invalid Port!\r\n");
            continue;
        }
        printf("%s%d: flags=%d<> mtu %d\r\n", port->name, port->num, port->flags, port->mtu);
        printf("        inet %s  netmask %s  broadcast %s\r\n", ip4addr_ntoa(netif_ip4_addr(port)), ip4addr_ntoa(&port->netmask), ip4addr_ntoa(&port->netmask));
        /* printf("        inet6 %s  prefixlen %d  scopeid 0x%02x\r\n", ip6_addr_ntoa(&port->ip6_addr), port->ip6_addr.prefixlen, port->ip6_addr.scopeid); */
        printf("        ether %02x:%02x:%02x:%02x:%02x:%02x\r\n", port->hwaddr[0], port->hwaddr[1], port->hwaddr[2], port->hwaddr[3], port->hwaddr[4], port->hwaddr[5]);
        printf("        RX packets %d  bytes   %d\r\n", port->mib2_counters.ifinucastpkts, port->mib2_counters.ifinoctets);
        printf("        RX errors  %d  dropped %d  unknown %d\r\n", port->mib2_counters.ifinerrors, port->mib2_counters.ifindiscards, port->mib2_counters.ifinunknownprotos);
        printf("        TX packets %d  bytes   %d\r\n", port->mib2_counters.ifoutucastpkts, port->mib2_counters.ifoutoctets);
        printf("        TX errors  %d  dropped %d\r\n", port->mib2_counters.ifouterrors, port->mib2_counters.ifoutdiscards);
    }
    return 0;
}
SHELL_CMD_EXPORT_ALIAS(cmd_ifconfig, ifconfig, show network interface information);
...
uint32_t ping(char *target_name, uint16_t interval, uint16_t size, uint32_t count)
{
    ...
    if (lwip_setsockopt(s, SOL_SOCKET, SO_RCVTIMEO, &timeout, sizeof(timeout)) < 0) {
        printf("ping: set socket option failed\n\r");
        lwip_close(s);
        return -1;
    }
